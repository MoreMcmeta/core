...
public final class MockResourceManager implements ResourceManager {
    ...
    public Optional<Resource> getResource(ResourceLocation resourceLocationIn) {
        ...
        if (isMissing) {
            ...
            return Optional.of(new MockSimpleResource(lastPackName));
        }
    }

    @Override
    public List<Resource> getResourceStack(ResourceLocation resourceLocationIn) {
        return ImmutableList.of();
    }

    @Override
    public Map<ResourceLocation, Resource> listResources(String pathIn, Predicate<ResourceLocation> filter) {
        ...
        FILES_TO_ADD.forEach((fileName) -> {
            ...
            if (namespaceAndFile.length == 1) {
                locations.add(new ResourceLocation("minecraft", pathIn + "/" + namespaceAndFile[0]));
            } else {
                locations.add(new ResourceLocation(namespaceAndFile[0], pathIn + "/" + namespaceAndFile[1]));
            }

        });
        locations.removeIf(namespaceAndPath -> filter.negate().test(namespaceAndPath));

        return locations.stream().collect(Collectors.toMap(
                Function.identity(),
                (key) -> getResource(key).orElseThrow()
        ));
    }

    @Override
    public Map<ResourceLocation, List<Resource>> listResourceStacks(String pathIn, Predicate<ResourceLocation> filter) {
        List<ResourceLocation> locations = new ArrayList<>();
     ...
     * @author soir20
     */
    public static final class MockSimpleResource extends Resource {
        public MockSimpleResource(String packName) {
            super(packName, () -> EMPTY_STREAM);
        }
    }
